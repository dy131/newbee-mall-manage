...
public class ShopGoodsCategoryController {
    ...
    @Resource
    private ShopCategoryService newBeeMallCategoryService;
    ...
    public Result listForSelect(@RequestParam("categoryId") Long categoryId) {
        ...
        if (category.getCategoryLevel() == ShopCategoryLevelEnum.LEVEL_ONE.getLevel()) {
            ...
            List<GoodsCategory> secondLevelCategories = newBeeMallCategoryService.queryShopCategoryLevelByParentId(Collections.singletonList(categoryId), ShopCategoryLevelEnum.LEVEL_TWO.getLevel());
            if (!CollectionUtils.isEmpty(secondLevelCategories)) {
                //查询二级分类列表中第一个实体的所有三级分类
                List<GoodsCategory> thirdLevelCategories = newBeeMallCategoryService.queryShopCategoryLevelByParentId(Collections.singletonList(secondLevelCategories.get(0).getId()), ShopCategoryLevelEnum.LEVEL_THREE.getLevel());
        ...
        if (category.getCategoryLevel() == ShopCategoryLevelEnum.LEVEL_TWO.getLevel()) {
            ...
            List<GoodsCategory> thirdLevelCategories = newBeeMallCategoryService.queryShopCategoryLevelByParentId(Collections.singletonList(categoryId), ShopCategoryLevelEnum.LEVEL_THREE.getLevel());
            categoryResult.put("thirdLevelCategories", thirdLevelCategories);
        }
